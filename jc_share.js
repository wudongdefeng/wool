...
cron 15 6,12,18 * * * jc_share.js

[rewrite_local]

#火爆转发
https://ant.xunsl.com/v17/NewTask/getTaskList.json 重写目标 https://raw.githubusercontent.com/shaolin-kongfu/js_scripts/main/jc_share.js
[MITM]
...
function share(wzid,timeout=0) {
    ...
    'Host': 'ant.xunsl.com',
     'Referer': 'https://ant.xunsl.com/h5/20200612makeMoney/?' +jc_cookie1
            ...
            body :  jc_cookie1 + '&article_id='+wzid,}
        $.post(url, async (err, resp, data) => {
            try {
...
function sharejl(timeout=0) {
    return new Promise((resolve) => {
        ...
        let url = {
            url : 'https://ant.xunsl.com/WebApi/ShareNew/execExtractTask',
            headers : {
    'Content-Type': 'application/x-www-form-urlencoded',
    'Content-Length': (jc_cookie1+ '&action=beread_extra_reward_'+ reward).length.toString(),
    'Host': 'ant.xunsl.com',
     'Referer': 'https://ant.xunsl.com/h5/20200612makeMoney/?' +jc_cookie1
},
            body :  jc_cookie1 + '&action=beread_extra_reward_'+ reward,}
        $.post(url, async (err, resp, data) => {
            try {
