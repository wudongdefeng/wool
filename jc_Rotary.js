...
!(async () => {
        console.log(`共${jc_cookieArr.length}个cookie`)
	        for (let k = 0; k < jc_cookieArr.length; k++) {
                ...
                for(let k = 0 ; k < 100 ; k++){
                    await Rotary(jc_cookie1,cookie_id,time)
                    await $.wait(6000);
                    console.log("\n\n")
                }
                for (let k = 1 ; k < 5 ; k++){
                    id = k.toString()
                    await openbox(jc_cookie1,cookie_id,time,id)
                    await $.wait(15000)
                }
                console.log("\n\n")
            }
...
function openbox(jc_cookie1,cookie_id,time,k,timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            url : 'https://ant.xunsl.com/WebApi/RotaryTable/chestReward?_='+ time,
            headers : {'Host': 'ant.xunsl.com',
