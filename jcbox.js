...
if (!jcboxbody) {
    ...
    Object.keys(jcboxbodys).forEach((item) => {
        if (jcboxbodys[item]) {
            jcboxbodyArr.push(jcboxbodys[item])
        }
    })
    ...
    if ($request.url.match(/\/ant.xunsl.com\/v5\/CommonReward\/toGetReward/)) {
        ...
        if (jcboxbody) {
            if (jcboxbody.indexOf(bodyVal) > -1) {
                $.log("此宝箱请求已存在，本次跳过")
            } else if (jcboxbody.indexOf(bodyVal) == -1) {
                jcboxbodys = jcboxbody + "&" + bodyVal;
                $.setdata(jcboxbodys, 'jcboxbody');
                $.log(`${$.name}获取宝箱: 成功, jcboxbodys: ${bodyVal}`);
                bodys = jcboxbodys.split("&")
                $.msg($.name, "获取第" + bodys.length + "个宝箱请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'jcboxbody');
            $.log(`${$.name}获取宝箱: 成功, jcboxbodys: ${bodyVal}`);
